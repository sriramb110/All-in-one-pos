...
function MyProfile({ profile }: ProfileProps) {
    ...
    const personalDetails: PersonalDetail[] = [
        ...
        { value: "phoneNumber", header: "Phone number", content: profile.phoneNumber },
        { value: "emailId", header: "Email ID", content: profile.emailId },
        { value: "addressLine1", header: "Address line 1", content: profile.addressLine1, editable: true },
        { value: "addressLine2", header: "Address line 2", content: profile.addressLine2 ?? "", editable: true },
        { value: "district", header: "District", content: profile.district, editable: true },
        { value: "state", header: "State", content: profile.state, editable: true },
        { value: "country", header: "Country", content: profile.country, editable: true },
    ];
    ...
    const isDataChanged = () => {
        return Object.keys(editableProfile).some(
            (key) => editableProfile[key as keyof Profiles] !== profile[key as keyof Profiles]
        );
    };

    const putProfileData = ()=>{

        putProfile(editableProfile).then((res)=>{console.log(res)}).catch((error)=>console.error(error))
        ...
        <div className="w-full h-full -mt-5 flex justify-center items-center overflow-hidden">
            <div className="w-1/2 h-5/6 border flex flex-col justify-center items-center border-blue-200 rounded-3xl shadow-2xl overflow-hidden">
                <div className="w-full h-fit flex p-1">
                    ...
                    <div className="w-full h-full flex flex-col justify-center items-center">
                        ...
                        <div className="gap-2 pt-2">
                            <button onClick={editEnabile} className="px-4 py-2 bg-blue-500 text-white rounded-full m-2 hover:bg-blue-700">
                                {isEditing ? 'Cancel EDIT' : 'EDIT'}
                            </button>
                            <button className="px-4 py-2 bg-green-500 text-white rounded-full m-2 hover:bg-green-700">MEMBERSHIP</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="w-full flex justify-center items-center h-1/4">
                    <button
                        className={isEditing && isDataChanged() ? "confirm" : "confirm_dissable"}
