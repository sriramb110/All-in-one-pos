...
function Headers({ hBgColor }: Colors) {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setBussnessName(sessionStorage.getItem("businessName"));
    };
  ...
  const icons = (IconComponent: React.ElementType) => {
    return <IconComponent className="mb-5 mr-5 w-5 h-5" />;
  };
  const headerName = () => {
    ...
    <div className={`w-auto h-16 ${hBgColor} p-4 text-white flex`}>
      <div className="pl-5 flex justify-between w-full pr-10">
        ...
        <div className="flex w-2/6 justify-end ">
          <div className=" flex pl-4 py-2 justify-start items-center">
            <h1>{bussnessName}</h1>
